...
void TextEdit::setFonts(QString fonts, int size, QColor color, QString style)
{
    mFontFamily = fonts;
    mFontSize = size;
    mLineNumFont = QFont(fonts.section(',', 0, 0), size);
    // 文本显示
    ...
    setHighLight(mLanguage); // 重新设置语法高亮
}

void TextEdit::setMarginsWidth()
{
    QString str = QString::number(mLineCount);
    QFontMetrics fm(mLineNumFont);
...
void TextEdit::onTextChanged()
{
    ...
    if (mScrollEnd && mIsWrap) {
        int lineEnd = mLineCount - 1; // 末行
...
void TextEdit::onLinesChanged()
{
    ...
    mLineCount = SendScintilla(SCI_GETLINECOUNT);
    temp = countOrder(mLineCount);
    if (temp != order) {
        order = temp;
        setMarginsWidth();
    }
